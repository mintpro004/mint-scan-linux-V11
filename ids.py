"""
Mint Scan v11.1 — IDS/IPS Integration (Suricata / Snort)
...
class IDSScreen(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        Btn(rc_card, '▶ TEST SYNTAX', command=self._test_rule, width=150).pack(pady=(0, 4))
        Btn(rc_card, '🧙 RULE WIZARD', command=self._show_wizard, variant='ghost', width=150).pack(pady=(0, 10))
        self._rule_status = ctk.CTkLabel(rc_card, text='',
                                          font=MONO_SM, text_color=C['mu'])
        self._rule_status.pack(pady=(0, 8))

    def _show_wizard(self):
        RuleWizardModal(self, on_generate=self._apply_wizard_rule)

    def _apply_wizard_rule(self, rule):
        self._rule_entry.delete(0, 'end')
        self._rule_entry.insert(0, rule)
        self._rule_status.configure(text="Rule generated by wizard.", text_color=C['ac'])
                    ...
                    if alerts:
                        ...
                        if not ids['suricata'] and not ids['snort']:
                            ...
                            msg = (f'IDS installed but STOPPED.\n\n'
                                   '→ Tap ▶ START in the sections above.\n'
                                   '→ Listening interface: {self._get_iface()}\n\n'
                                   'Real-time alerts will appear here as threats are detected.')
                        else:
...
class RuleWizardModal(ctk.CTkToplevel):
    def __init__(self, parent, on_generate):
        super().__init__(parent)
        self.title("Suricata Rule Wizard")
        self.geometry("500x550")
        self.configure(fg_color=C['bg'])
        self.attributes('-topmost', True)
        self.on_generate = on_generate

        inner = ScrollableFrame(self)
        inner.pack(fill='both', expand=True, padx=20, pady=20)
        
        ctk.CTkLabel(inner, text="SURICATA RULE WIZARD", 
                     font=('DejaVu Sans Mono', 14, 'bold'), text_color=C['ac']).pack(pady=(0,20))

        # Fields
        self.action = self._add_field(inner, "ACTION", ["alert", "drop", "pass", "reject"])
        self.proto  = self._add_field(inner, "PROTOCOL", ["tcp", "udp", "icmp", "ip", "http", "dns", "tls"])
        self.src_ip = self._add_entry(inner, "SOURCE IP", "any")
        self.src_pt = self._add_entry(inner, "SOURCE PORT", "any")
        self.dst_ip = self._add_entry(inner, "DEST IP", "any")
        self.dst_pt = self._add_entry(inner, "DEST PORT", "any")
        self.msg    = self._add_entry(inner, "MESSAGE (msg)", "Threat detected")
        self.sid    = self._add_entry(inner, "RULE ID (sid)", "1000001")

        Btn(inner, "🪄 GENERATE RULE", command=self._generate, width=200).pack(pady=20)
        Btn(inner, "CANCEL", command=self.destroy, variant='ghost', width=100).pack()

    def _add_field(self, parent, label, options):
        f = ctk.CTkFrame(parent, fg_color='transparent')
        f.pack(fill='x', pady=4)
        ctk.CTkLabel(f, text=label, font=MONO_SM, text_color=C['mu'], width=120, anchor='w').pack(side='left')
        var = ctk.StringVar(value=options[0])
        ctk.CTkOptionMenu(f, variable=var, values=options, fg_color=C['s2'], 
                          button_color=C['br2'], font=MONO_SM, width=150).pack(side='left')
        return var
    ...
    def _add_entry(self, parent, label, default):
        ...
        f.pack(fill='x', pady=4)
        ctk.CTkLabel(f, text=label, font=MONO_SM, text_color=C['mu'], width=120, anchor='w').pack(side='left')
        e = ctk.CTkEntry(f, font=MONO_SM, fg_color=C['bg'], border_color=C['br'], height=28)
        e.pack(side='left', fill='x', expand=True)
        e.insert(0, default)
        return e

    def _generate(self):
        rule = (f"{self.action.get()} {self.proto.get()} {self.src_ip.get()} {self.src_pt.get()} -> "
                f"{self.dst_ip.get()} {self.dst_pt.get()} "
                f"(msg:\"{self.msg.get()}\"; sid:{self.sid.get()}; rev:1;)")
        self.on_generate(rule)
        self.destroy()
